"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AdminService = void 0;
const client_1 = require("@prisma/client");
const admin_constant_1 = require("./admin.constant");
const pagination_1 = __importDefault(require("../../../helper/pagination"));
const prisma_1 = __importDefault(require("../../../shared/prisma"));
const getAllAdmin = (params, options) => __awaiter(void 0, void 0, void 0, function* () {
    const { searchTerm } = params, filterData = __rest(params, ["searchTerm"]);
    const { page, limit, skip, sortBy, sortOrder } = (0, pagination_1.default)(options);
    const andConditions = [];
    if (searchTerm) {
        andConditions.push({
            OR: admin_constant_1.adminSearchableFields.map((field) => ({
                [field]: {
                    contains: searchTerm,
                    mode: "insensitive",
                },
            })),
        });
    }
    if (Object.keys(filterData).length > 0) {
        andConditions.push({
            AND: Object.keys(filterData).map((key) => ({
                [key]: {
                    equals: filterData[key],
                },
            })),
        });
    }
    andConditions.push({
        isDeleted: false,
        user: {
            role: {
                in: ["ADMIN", "SUPER_ADMIN"],
            },
        },
    });
    const whereConditions = { AND: andConditions };
    const result = yield prisma_1.default.userProfile.findMany({
        where: whereConditions,
        skip,
        take: limit,
        orderBy: sortBy && sortOrder ? { [sortBy]: sortOrder } : { createdAt: "desc" },
        include: {
            user: true,
        },
    });
    const total = yield prisma_1.default.userProfile.count({
        where: whereConditions,
    });
    return {
        meta: {
            page,
            limit,
            total,
        },
        data: result,
    };
});
const getSingleAdminDataById = (id) => __awaiter(void 0, void 0, void 0, function* () {
    yield prisma_1.default.userProfile.findUniqueOrThrow({
        where: {
            id: id,
            user: {
                role: client_1.UserRole.SUPER_ADMIN || client_1.UserRole.ADMIN,
            },
        },
    });
    const result = yield prisma_1.default.userProfile.findUnique({
        where: {
            id: id,
            isDeleted: false,
        },
    });
    return result;
});
const updateAdminDataById = (id, data) => __awaiter(void 0, void 0, void 0, function* () {
    yield prisma_1.default.userProfile.findUniqueOrThrow({
        where: {
            id: id,
            isDeleted: false,
        },
    });
    const result = yield prisma_1.default.userProfile.update({
        where: {
            id: id,
        },
        data,
    });
    return result;
});
const deleteAdminDataByID = (id) => __awaiter(void 0, void 0, void 0, function* () {
    yield prisma_1.default.userProfile.findUniqueOrThrow({
        where: {
            id: id,
        },
    });
    const result = yield prisma_1.default.$transaction((txClient) => __awaiter(void 0, void 0, void 0, function* () {
        const deletedAdminData = yield txClient.userProfile.delete({
            where: {
                id: id,
            },
        });
        yield txClient.user.delete({
            where: {
                email: deletedAdminData.email,
            },
        });
        return deletedAdminData;
    }));
    return result;
});
const softlyDeleteAdminDataByID = (id) => __awaiter(void 0, void 0, void 0, function* () {
    yield prisma_1.default.userProfile.findUniqueOrThrow({
        where: {
            id: id,
            isDeleted: false,
        },
    });
    const result = yield prisma_1.default.$transaction((txClient) => __awaiter(void 0, void 0, void 0, function* () {
        const deletedAdminData = yield txClient.userProfile.update({
            where: {
                id: id,
            },
            data: {
                isDeleted: true,
            },
        });
        yield txClient.user.update({
            where: {
                email: deletedAdminData.email,
            },
            data: {
                status: client_1.UserStatus.DELETED,
            },
        });
        return deletedAdminData;
    }));
    return result;
});
exports.AdminService = {
    getAllAdmin,
    getSingleAdminDataById,
    updateAdminDataById,
    deleteAdminDataByID,
    softlyDeleteAdminDataByID,
};
// [
//     {
//       name: {
//         contains: params.searchTerm,
//         mode: "insensitive",
//       },
//     },
//     {
//       email: {
//         contains: params.searchTerm,
//         mode: "insensitive",
//       },
//     },
//   ],
